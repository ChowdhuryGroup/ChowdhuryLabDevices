...
def openShutter(duration,delay,probe_delay = 0):
    ...
    with nidaqmx.Task() as t:
        ...
        shutterOpenTime = int(float(duration)*float(samples_per_second)) #Number of samples in corresponding to shutter open length
        ...
        t.timing.cfg_samp_clk_timing(int(samples_per_second),source="/Dev1/PFI0",sample_mode=AcquisitionType.FINITE,samps_per_chan=int(samples_per_second))
        ...
        delay=int(delay)
        signallist1 = [0.0]*(1+delay)+[5.0]*shutterOpenTime+[0.0]*(samples_per_second-shutterOpenTime-delay-1)
        signallist2 = [0.0]*(1+probe_delay)+[5.0]*shutterOpenTime+[0.0]*(samples_per_second-shutterOpenTime-probe_delay-1)
